...
function getTodayTasks () {
  const allTasks = getFromLocalStorage('tasksArray')
  ...
  for (const task of allTasks) {
    ...
    const deadline = new Date(dateArray[2], dateArray[1] - 1, dateArray[0]).getTime()
...
function getTask (taskTitle) {
  const tasks = getFromLocalStorage('tasksArray')
  for (let i = 0; i < tasks.length; i++) { if (tasks[i].title == taskTitle) return tasks[i] }
...
function saveToLocalStorage (nameOfKey, object) {
  if (localStorage.getItem(nameOfKey) == null || localStorage.getItem(nameOfKey) === '') {
    ...
    const stringifiedArray = localStorage.getItem(nameOfKey)
...
function deleteFromLocalStorage (nameOfKey, objectTitleToRemove) {
  ...
  const objects = getObjectsFromStringifiedArray(localStorage.getItem(nameOfKey))
  for (let i = 0; i < objects.length; i++) {
    if (objects[i].title == objectTitleToRemove) { objects.splice(i, 1) }
  }
...
function convertObjectsToStringifiedArray (objectsArray) {
  ...
  for (let i = 0; i < objectsArray.length; i++) { stringifiedArray += convertObjectToStringifiedArray(objectsArray[i]) }
...
function getObjectsFromStringifiedArray (stringifiedArray) {
  ...
  const stringObjects = stringifiedArray.split(';')
  const objects = []
  for (const stringObject of stringObjects) { objects.push(JSON.parse(stringObject)) }
